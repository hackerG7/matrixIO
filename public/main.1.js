	...
	if(value.server_hostname.indexOf(Currenthostname)==-1){
		...
		for(var i = 0 ; i < 5 ; i++){
			...
			bt.button.mousePressed(function(){
				...
				sendMsg("levelup_ability",{abilityID:this.master.index})
			})
		...
		b.button.mousePressed(function(){
			sendMsg("start_game",{name:inputName.input.elt.value})
		})
	}
}
function draw(){
	...
	if(show_evolution){
		if(keyIsPressed){
			if(keyCode==ascii("1")||keyCode==ascii("2")||keyCode==ascii("3")||keyCode==ascii("4")){
				...
				sendMsg("evolution",{selectedID:selectedID})
			}
		}
	}
...
//camera zoom set
	...
	var me = get_myPlayer();
	if(me!=undefined){
		for(var i = 0 ; i < me.matrix_array.length ; i++){
			var m = me.matrix_array[i];
			sendMsg("matrix_update",{
				matrixID:i,
				x: m.position.x,
				y: m.position.y,
			})
...
function sendMouseData(key){
	...
	sendMsg("clicking",{key:key,x:camera.mouseX,y:camera.mouseY,distance:distance})
}
function player_control(){
	...
	if(up||left||down||right){
		...
		if(hspeed!=false || vspeed!=false){
			sendMsg("player_move",{direction:d})
		}
	}else{
		sendMsg("player_stop")
	}
